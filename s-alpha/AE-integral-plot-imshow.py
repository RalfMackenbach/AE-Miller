...
s_grid      =  np.linspace(-2.0, +2.0,   num=41 ,dtype='float64')
alpha_grid   = np.linspace(+0.0, +4.0,   num=41 ,dtype='float64')
...
if __name__ == "__main__":
    ...
    # ax  = fig.gca()
    plt.imshow(np.flip(AEv,0), extent=(alpha_grid.min(), alpha_grid.max(), s_grid.min(), s_grid.max()),
           interpolation='nearest', cmap='plasma')
    plt.colorbar(format=ticker.FuncFormatter(fmt),label=r'$\hat{A}^{3/2}$')
    ...
    # plt.margins(0.1)
    # plt.savefig('/Users/ralfmackenbach/Documents/GitHub/AE-tok/plots/sa_plot_-_wn={}_eta={}_q={}.eps'.format(wn_input,eta_input,q_input), format='eps',
