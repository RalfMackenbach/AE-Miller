...
if __name__ == "__main__":
    ...
    print('max error is',np.amax(rel_err))

    levels0 = np.linspace(0, 0.1, 25)

    fig, axs = plt.subplots(1,1, figsize=(6.850394/2, 5.0/2)) #figsize=(6.850394, 3.0)
    cnt0 = axs.contourf(alphav, sv, rel_err, levels=levels0, cmap='plasma')
    for c in cnt0.collections:
        c.set_edgecolor("face")
    cbar0 = fig.colorbar(cnt0,ticks=[0.0, 0.1],ax=axs)
    cbar0.ax.set_yticklabels([r'$0\%$',r'$10\%$']) # set ticks of your format
    cbar0.solids.set_edgecolor("face")

    cbar0.set_label(r'$\mathrm{Error}$')
    axs.set_xlabel(r'$\alpha$')
    axs.set_ylabel(r'$s$')

    # plt.text(3.2, -1.6, r'$(b)$',c='white')
    axs.xaxis.set_tick_params(which='major', direction='in', top='on')
    axs.xaxis.set_tick_params(which='minor', direction='in', top='on')
    axs.yaxis.set_tick_params(which='major', direction='in', top='on')
    axs.yaxis.set_tick_params(which='minor', direction='in', top='on')
    plt.tight_layout()
    # plt.subplots_adjust(left=0.15, right=0.88, top=0.96, bottom=0.14)
    plt.margins(0.1)
    plt.savefig('/Users/ralfmackenbach/Documents/GitHub/AE-tok/plots/s-alpha/code-comparison-error.eps', format='eps',
